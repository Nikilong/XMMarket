...
let getNewsByChannel = function(data) {
    let params = data.PARAMS
    let resData = {}
    let cityName= encodeURI("广州")
    if(params.cityName){
        cityName = params.cityName
    }
    console.log("---city:",cityName,params.id)
...
// 查询热点新闻
let getHotNews = function(data) {
    console.log("------",data.SERVICE)
    let resData = {}
    let weburl = `https://so.sina.cn/channel/360/search`
    return new Promise((resolve, reject) => {
        request(weburl, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                // console.log(body) // Show the HTML for the baidu homepage.
                ...
                try{
                    // 提取目标json
                    let reg = /{"word_list":(.*),"focuspic_list"/
                    dict = reg.exec(body)[1]
                }catch(e){console.log(e)}
                resData.status = 'success'
                resData.RESULT = dict
            }else{
